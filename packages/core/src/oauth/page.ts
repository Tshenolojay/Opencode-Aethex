...
// etc.).
//
// The visual language mirrors the OpenCode Aethex app: the design tokens are a curated
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `OpenCode Aethex is now connected to ${escapeHtml(provider)}.`
        : "OpenCode Aethex is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `OpenCode Aethex couldn't finish connecting to ${escapeHtml(provider)}.`
        : "OpenCode Aethex couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from OpenCode Aethex.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · OpenCode Aethex</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("OpenCode Aethex couldn't finish connecting to "+PROVIDER+"."):"OpenCode Aethex couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from OpenCode Aethex."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("OpenCode Aethex is now connected to "+PROVIDER+"."):"OpenCode Aethex is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
const STYLES = `
  ...
  * { box-sizing: border-box; }
  ...
  .brand svg { height: 22px; width: auto; max-width: 100%; }
...
// OpenCode Aethex wordmark — same path geometry as packages/ui/src/components/logo.tsx (Logo).
const WORDMARK = `<svg class="wordmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 360 42" fill="none" aria-label="OpenCode Aethex" role="img">
        ...
        <text x="250" y="29" fill="var(--oc-icon-strong)" font-family="ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif" font-size="22" font-weight="650" letter-spacing="0.06em">Aethex</text>
      </svg>`
